...
class DatacardFactory:
    ...
    _dnnVariablePattern = re.compile(r"evaluate_dnn_(ttDM|tWDM)_(s|ps)_(\d+)")
    _topDMSignalPattern = re.compile(
        r"^(TTto2LDMsimpSpin0|TWto2LDMsimpSpin0)_(s|ps)_mphi-(\d+)$"
    )
    ...
    def _getSignalPair(self, mediator, mass, alternative_signals):
        expected_signals = [
            f"TTto2LDMsimpSpin0_{mediator}_mphi-{mass}",
            f"TWto2LDMsimpSpin0_{mediator}_mphi-{mass}",
        ]
        return [signal for signal in expected_signals if signal in alternative_signals]

    def _getSignalConfigurations(self, variableName, alternative_signals):
        """Return ``(card suffix, signals in card)`` configurations.

        For a DNN variable there is one card, named after the production mode
        targeted by that DNN, but it contains both ttDM and tWDM at the same
        mediator type and mass.  This preserves the filenames expected by the
        limits workflow while making the statistical signal model complete.

        For non-DNN variables, keep the historical per-signal filenames.  Each
        top-DM filename nevertheless contains the corresponding ttDM+tWDM pair,
        so control-region cards also include both production modes.
        """
        match = self._dnnVariablePattern.fullmatch(variableName)
        if match:
            process, mediator, mass = match.groups()
            primary_prefix = (
                "TTto2LDMsimpSpin0" if process == "ttDM" else "TWto2LDMsimpSpin0"
            )
            primary_signal = f"{primary_prefix}_{mediator}_mphi-{mass}"
            signal_pair = self._getSignalPair(mediator, mass, alternative_signals)
            expected_pair = {
                f"TTto2LDMsimpSpin0_{mediator}_mphi-{mass}",
                f"TWto2LDMsimpSpin0_{mediator}_mphi-{mass}",
            }
            missing_signals = sorted(expected_pair - set(signal_pair))
            if missing_signals:
                raise RuntimeError(
                    f"DNN variable {variableName} requires both ttDM and tWDM signals; "
                    f"missing: {', '.join(missing_signals)}"
                )
            print(
                f"    DNN signal matching: {variableName} -> "
                + " + ".join(signal_pair)
            )
            return [(primary_signal, signal_pair)]

        configurations = [("", [])]
        for signal in alternative_signals:
            if not signal:
                continue
            signal_match = self._topDMSignalPattern.fullmatch(signal)
            if signal_match:
                _, mediator, mass = signal_match.groups()
                signals = self._getSignalPair(mediator, mass, alternative_signals)
            else:
                signals = [signal]
            configurations.append((signal, signals))
    ...
    def makeDatacards(
        ...
        for cutName in cuts:
            ...
            for variableName, variable in variables.items():
                ...
                is_signal_region = cutName.startswith("ttdm_sr_")
                if is_signal_region and "evaluate" not in variableName:
                    print(f"  Skipping {variableName} in {cutName}: signal regions use only evaluate variables")
                    continue
                if not is_signal_region and "evaluate" not in variableName and variableName != "events":
                    print(f"  Skipping {variableName} in {cutName}: control regions use only events")
                    continue

                print("  variableName = ", variableName)
                dnn_match = self._dnnVariablePattern.fullmatch(variableName)
                is_dnn_variable = dnn_match is not None

                if is_dnn_variable:
                    required_cut = "ttdm_sr_2l_1b" if dnn_match.group(1) == "tWDM" else "ttdm_sr_2l_2b"
                    if cutName != required_cut and not cutName.startswith(required_cut + "_"):
                        print(f"  Skipping {variableName} in {cutName}: only fitted in {required_cut}")
                        continue

                signal_configurations = self._getSignalConfigurations(variableName, alternative_signals)
                if is_dnn_variable and not signal_configurations:
                    print(f"  Skipping {variableName}: matching signal not available")
                    continue
                ...
                signals_to_write = list(cut_signals)
                if is_dnn_variable:
                    signals_to_write = []
                    for _, signal_group in signal_configurations:
                        signals_to_write += [
                            signal
                            for signal in signal_group
                            if signal in cut_signals and signal not in signals_to_write
                        ]
                    signals_to_write += [signal for signal in cumulative_signals if signal in cut_signals and signal not in signals_to_write]

                for snameList, yields in [
                    (signals_to_write, yieldsSig),
                ...
                # One card per signal hypothesis. For top-DM hypotheses, each
                # card contains both ttDM and tWDM at the same model and mass.
                for signalName, alternativeSignalGroup in signal_configurations:
                    if signalName == "":
                        signals = [
                        ...
                        selectedAlternativeSignals = [
                            name for name in alternativeSignalGroup if name in cut_signals
                        ]
                        if is_dnn_variable and len(selectedAlternativeSignals) != len(alternativeSignalGroup):
                            missingFromCut = sorted(set(alternativeSignalGroup) - set(selectedAlternativeSignals))
                            raise RuntimeError(
                                f"DNN variable {variableName} in cut {cutName} requires both "
                                f"signal modes; removed from this cut: {', '.join(missingFromCut)}"
                            )
                        signals = selectedAlternativeSignals + [
                            name for name in cumulative_signals if name in cut_signals
                        ]
                    ...
                    for nuisance in nuisances.values():
                        ...
                        if "range" in nuisance:
                            paramRange = nuisance["range"]
                            if not isinstance(paramRange, (list, tuple)) or len(paramRange) != 2:
                                raise RuntimeError(
                                    "Invalid rateParam range for %s: expected [min, max], got %s"
                                    % (nuisance["name"], paramRange)
                                )

                            rangeMin, rangeMax = map(float, paramRange)
                            if rangeMin >= rangeMax:
                                raise RuntimeError(
                                    "Invalid rateParam range for %s: min >= max (%s)"
                                    % (nuisance["name"], paramRange)
                                )

                            card.write(("[%g,%g]" % (rangeMin, rangeMax)).ljust(columndef))
