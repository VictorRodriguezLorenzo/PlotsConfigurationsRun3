...
class DatacardFactory:
    ...
    def _getSignalsForVariable(self, variableName, alternative_signals):
        match = self._dnnVariablePattern.fullmatch(variableName)
        if not match:
            return alternative_signals

        process, mediator, mass = match.groups()
        prefix = "TTto2LDMsimpSpin0" if process == "ttDM" else "TWto2LDMsimpSpin0"
        expected_signal = f"{prefix}_{mediator}_mphi-{mass}"
        matched_signals = [signal for signal in alternative_signals if signal == expected_signal]

        if matched_signals:
            print(f"    DNN signal matching: {variableName} -> {expected_signal}")
        else:
            print(f"WARNING: no signal {expected_signal} found for {variableName}")
    ...
    def makeDatacards(
        ...
        for cutName in cuts:
            ...
            for variableName, variable in variables.items():
                ...
                variable_alternative_signals = self._getSignalsForVariable(variableName, alternative_signals)
                if is_dnn_variable and not variable_alternative_signals:
                ...
                if is_dnn_variable:
                    signals_to_write = [signal for signal in variable_alternative_signals if signal in cut_signals]
                ...
                for signalName in variable_alternative_signals:
                    if signalName == "":
                        signals = [
