...
if __name__ == '__main__':
    ...
    for sam_k, sam_v in samples.items():
        isSignal = sam_v.get('isSignal', 'DM' in sam_k)
        isTTZ = sam_k.startswith('TTZ') or sam_k.startswith('ttZ')
        isDY = sam_k.startswith('DY')
    
        if not (isSignal or isTTZ or isDY):
            continue
