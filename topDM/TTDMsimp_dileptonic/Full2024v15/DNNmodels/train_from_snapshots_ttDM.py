...
SIGNAL_TYPES = ("ps", "s")
SIGNAL_TYPE = os.environ.get("TTDM_SIGNAL_TYPE")
SIGNAL_PRODUCTION_MODES = ("ttDM", "tWDM")
SIGNAL_PREFIXES = {
    "ttDM": "TTto2LDMsimpSpin0",
    "tWDM": "TWto2LDMsimpSpin0",
}
SIGNAL_MODE_BATCH_FRACTIONS = {"ttDM": 0.50, "tWDM": 0.50}
...
if SIGNAL_TYPE is None:
    ...
    for signal_type in SIGNAL_TYPES:
        print("\n" + "=" * 80)
        print(f"Starting ttDM training for signal type: {signal_type}")
        print("=" * 80)

        env = os.environ.copy()
        env["TTDM_SIGNAL_TYPE"] = signal_type
        subprocess.run([sys.executable, os.path.abspath(__file__)], env=env, check=True)

    print("\nFinished ttDM trainings for: ps, s")
    sys.exit(0)

if SIGNAL_TYPE not in SIGNAL_TYPES:
    raise ValueError(
        f"Unknown TTDM_SIGNAL_TYPE={SIGNAL_TYPE!r}. "
        f"Expected one of {SIGNAL_TYPES}."
    )

ANALYSIS_NAME = f"model_DNN_ttDM_{SIGNAL_TYPE}"
save_model = True
...
MODEL_NAME = os.path.join(SCRIPT_DIR, f"Models/model_DNN_ttDM_{SIGNAL_TYPE}.keras")
...
SNAPSHOT_DIR = os.environ.get(
    "TOPDM_SNAPSHOT_DIR",
    os.path.join(SCRIPT_DIR, "files_for_training"),
)
CATEGORY_SELECTION = "nbjets > 1 && tt_reco"
...
#   (ttDM + tWDM) vs (ttbar + single top + visible ttV)
...
# to decide whether the new training preserves signal-vs-tt(2l) separation,
...
# The previous dedicated TTNuNu reference used a ttDM-only signal class and is
# therefore not directly comparable with this mixed ttDM+tWDM training.
...
# Give the new training a different output name by default so the current
# model is not overwritten accidentally.
if ANALYSIS_NAME == "dnn_model_all_variables":
    if TRAINING_TARGET == "main":
        ANALYSIS_NAME = "dnn_model_all_variables_paperAdapted"
    else:
        ANALYSIS_NAME = "dnn_model_ttNuNu_paperAdapted"
...
def extract_signal_mode(sample_tag):
    for mode, prefix in SIGNAL_PREFIXES.items():
        if f"{prefix}_{SIGNAL_TYPE}_" in sample_tag:
            return mode
    return None
...
SIGNAL_PATTERNS = tuple(
    f"{SIGNAL_PREFIXES[mode]}_{SIGNAL_TYPE}_"
    for mode in SIGNAL_PRODUCTION_MODES
)
files_sig = [
    f for f in files
    if any(pattern in os.path.basename(f) for pattern in SIGNAL_PATTERNS)
]
...
for mode, pattern in zip(SIGNAL_PRODUCTION_MODES, SIGNAL_PATTERNS):
    print(f"    {mode:10s}:          {sum(pattern in os.path.basename(f) for f in files_sig)}")
...
def dataframe_from_files(file_list, columns, description):
    ...
    df = df.Filter(CATEGORY_SELECTION)

    n_entries = df.Count().GetValue()
    print(
        f"Reading {description}: {len(file_list)} files, "
        f"{n_entries} entries after {CATEGORY_SELECTION}"
    )
...
for sample_tag, file_list in sorted(signal_file_groups.items()):
    ...
    signal_mode = extract_signal_mode(sample_tag)
    if signal_mode is None:
        raise RuntimeError(f"Could not infer signal production mode from: {sample_tag}")
...
print("\nSignal entries per mass and production mode:")
print(Sig.groupby(["mPhi_true", "signal_mode"]).size())
...
df_all.loc[signal_split_mask, "split_stratum"] = (
    ...
    + "_"
    + df_all.loc[signal_split_mask, "signal_mode"].astype(str)
)
...
def signal_counts_by_hypothesis(total):
    """Split signal draws over mPhi and configured production-mode fractions."""
    mass_counts = integer_partition(total, len(signal_hypotheses))
    counts = {}
    for i_mass, (mass, mass_count) in enumerate(zip(signal_hypotheses, mass_counts)):
        modes = list(SIGNAL_PRODUCTION_MODES)
        if i_mass % 2:
            modes.reverse()
        mode_fractions = normalized_group_fractions(
            modes,
            requested_fractions=SIGNAL_MODE_BATCH_FRACTIONS,
        )
        for mode, count in counts_from_fractions(mass_count, mode_fractions).items():
            counts[(mass, mode)] = count
    return counts
...
def full_balanced_batch_generator(
    ...
    Full balance:
      ...
      2) signal half balanced over all mPhi hypotheses and production modes;
    ...
    signal_indices_by_hypothesis = {}
    for mass in signal_hypotheses:
        for mode in SIGNAL_PRODUCTION_MODES:
            idx = frame.index[
                (frame["isSignal"] == 1)
                & np.isclose(frame["mPhi_true"], mass)
                & (frame["signal_mode"] == mode)
            ].to_numpy()

            if len(idx) == 0:
                raise RuntimeError(
                    f"No training signal events for mPhi={mass}, mode={mode}."
                )

            signal_indices_by_hypothesis[(mass, mode)] = idx
    ...
    sig_counts = signal_counts_by_hypothesis(signal_half)
    ...
    print("  Signal events per mass and production mode:")
    for (mass, mode), count in sig_counts.items():
        print(f"    mPhi={mass:g}, {mode}: {count}")
    ...
    while True:
        ...
        # Signal balance: every mass and production mode is in every batch.
        for hypothesis, count in sig_counts.items():
            pool = signal_indices_by_hypothesis[hypothesis]
...
def print_epoch_sampling_diagnostics(frame, steps, batch_size, background_fractions):
    ...
    signal_counts = signal_counts_by_hypothesis(signal_half)
    ...
    for (mass, mode), per_batch in signal_counts.items():
        pool = int(np.sum(
            (frame["isSignal"] == 1)
            & np.isclose(frame["mPhi_true"], mass)
            & (frame["signal_mode"] == mode)
        ...
        print(
            f"    mPhi={mass:g}, {mode}: draws={draws:6d}, "
...
def equal_signal_mode_weights(frame):
    """Give ttDM and tWDM equal total positive-class weight in diagnostics."""
    weights = np.ones(len(frame), dtype=float)
    for mode in SIGNAL_PRODUCTION_MODES:
        mask = (
            (frame["isSignal"].to_numpy(dtype=int) == 1)
            & (frame["signal_mode"].to_numpy(dtype=str) == mode)
        )
...
def evaluate_mass_hypothesis(
    ...
    evaluation_weights = equal_signal_mode_weights(evaluation_frame)
    auc_mass = roc_auc_score(y_mass, scores, sample_weight=evaluation_weights)
    fpr_mass, tpr_mass, thresholds_mass = metrics.roc_curve(
        y_mass,
        scores,
...
training_config = {
    ...
    "signal_production_modes": SIGNAL_PRODUCTION_MODES,
    "signal_batch_balance": "equal_per_mass_and_production_mode",
    "signal_mode_batch_fractions": SIGNAL_MODE_BATCH_FRACTIONS,
...
# not degrade the mixed-signal-vs-TTTo2L2Nu separation.
...
for mass in signal_hypotheses:
    ...
    for group, group_frame in background_group_auc_df.groupby("eval_group"):
        ...
        auc_group = roc_auc_score(
            y_group_auc,
            scores_group_auc,
            sample_weight=equal_signal_mode_weights(evaluation_frame),
        )
...
if MAKE_SUMMARY_DIAGNOSTIC_PLOTS:
    ...
    if "background_group_auc_table" in globals() and len(background_group_auc_table) > 0:
        ...
        plt.grid(True, alpha=0.3)
        plt.legend(ncol=2)
        plt.title("ttDM+tWDM discrimination against each background")
        ...
        if "tt2l" in auc_pivot.columns:
            ...
            plt.ylim(0.5, 1.01)
            plt.grid(True, alpha=0.3)
            plt.title(r"Main task protection: ttDM+tWDM vs $t\bar{t}(2\ell)$")
            save_summary_plot("AUC_signal_vs_tt2l")
