...
class MergerFactory:
    ...
    def __init__(self, tag, inDir, outDir, foldersToMerge):
        ...
        self._fileOut = os.path.join(outDir, f"mkShapes__{tag}.root")
        self._filesIn = {}
        self.all_nuisances = {}
        ...
        for folderHR, folder in foldersToMerge.items():
            ...
            root_file_name = os.path.join(
                folder["outputFolder"],
                f"mkShapes__{folder['tag']}.root"
            )
        
            print(" Input ROOT file = ", root_file_name)
        
            self._filesIn[folderHR] = root_file_name
        
        print("Input ROOT files: ", self._filesIn)

    def extract_dict(self, filename, var_name, samples_input={}, cuts_input=None):
        namespace = {'os' : os,
                'samples': samples_input}
    
        if cuts_input is not None:
            namespace['cuts'] = cuts_input
        ...
        try:
            with open(filename, 'r') as f:
                exec(compile(f.read(), filename, "exec"), namespace, namespace)
    
            result = namespace.get(var_name)
    
            if result is None:
                raise RuntimeError(f"'{var_name}' not defined in {filename}")
    
            return result
    
        except Exception as e:
            raise RuntimeError(
                f"Error reading {filename}: {type(e).__name__}: {e}"
            ) from e
    ...
    def getVariedHistos(self, folderHR, folder_name, rootFileIn, sampleName, nuisanceName, nuisance, yearKeys, extra="CMS_") :
        ...
        if nameTempUp in yearKeys and nameTempDown in yearKeys :
            ...
            #
            if self.all_nuisances[folderHR] is None:
                raise RuntimeError(f"NUISANCES ARE NONE FOR folderHR = {folderHR}")
    ...
    def process_cut_variable(self, cutName, variableName, samples, new_list_of_nuisances):
        ...
        ## Open files and store all keys using ROOT
        allKeys = {}
        rootIn = {}
        ...
        for folderHR in self._filesIn.keys():
            ...
            if not rootIn[folderHR] or rootIn[folderHR].IsZombie():
                raise RuntimeError(f"FAILED opening ROOT file: {fileIn}")
        
            folder = rootIn[folderHR].Get(folder_name)
        
            if not folder:
                raise RuntimeError(
                    f"Folder '{folder_name}' not found in file: {fileIn}"
                )
        
            allKeys[folderHR] = np.unique([
                key.GetName()
    ...
    def merge(
        ...
        for folderHR, year_nuisances in foldersToMergeNuisancesFiles.items() :
            ...
            nuisances = self.extract_dict(year_nuisances, "nuisances", temp_samples, cuts)
...
def main():
    ...
    print("merge tag             = ", mergeTag)
    print("foldersToMerge        = ", opt.foldersToMerge)
    
    foldersToMergeNuisancesFiles = {
        k: os.path.join(p["folder"], "nuisances_ALL.py")
        for k, p in opt.foldersToMerge.items()
    }
    print("foldersToMergeNuisancesFiles = ", foldersToMergeNuisancesFiles)
    
    factory = MergerFactory(mergeTag, inDir, outDir, opt.foldersToMerge)
    factory.merge(
        mergeTag,
        variables,
        cuts,
