...
useDYHT = False   # True = MLL+HT DY, False = inclusive DY samples
...
if useDYHT:
    ...
    for mll in dy_mll_bins:
        for ht in dy_ht_bins:
            sample = f'DYto2L-4Jets_{mll}_{ht}'
            filesDYHT += nanoGetSampleFiles(mcDirectory, sample)

    ## Inclusive flavour-split samples used only for HT < 40
    dy_lowht_samples = [
        'DYto2E-2Jets_MLL-50',
    ...
    for sample in dy_lowht_samples:
        filesDYHT += nanoGetSampleFiles(mcDirectory, sample)
    ...
    # Keep only the low-HT part of the inclusive samples.
    # HT >= 40 is already covered by the MLL+HT-binned samples.
    for sample in dy_lowht_samples:
        addSampleWeight(samples, 'DY', sample, 'LHE_HT < 40.0')

else:
    ## Inclusive flavour-split samples
    filesDYInc = []

    dy_inclusive_samples = [
        'DYto2E-2Jets_MLL-50',
        'DYto2Mu-2Jets_MLL-50',
        'DYto2Tau-2Jets_MLL-50',
        'DYto2E-2Jets_MLL-10to50',
    ...
    for sample in dy_inclusive_samples:
        filesDYInc += nanoGetSampleFiles(mcDirectory, sample)
...
for _, sd in DataRun:
    for pd in DataSets:
        Datatag = f"{pd}_{sd}"
        files = nanoGetSampleFiles(fake_dir_map[pd], Datatag)
