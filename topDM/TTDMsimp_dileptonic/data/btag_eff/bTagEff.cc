...
void bTagEff_fix(std::string campaign, std::string WP="medium") {
    ...
    // Binning: use double precision to avoid TH2F saturation with tiny weights
    Double_t ptbins[]  = {20,30,50,70,100,140,200,300,600,1000};
    Double_t etabins[] = {-2.5,-1.479,1.479,2.5};

    TH2D *bjet_den = new TH2D("bjet_den","bjet_den",9,ptbins,3,etabins);
    TH2D *bjet_num = new TH2D("bjet_num","bjet_num",9,ptbins,3,etabins);
    TH2D *cjet_den = new TH2D("cjet_den","cjet_den",9,ptbins,3,etabins);
    TH2D *cjet_num = new TH2D("cjet_num","cjet_num",9,ptbins,3,etabins);
    TH2D *ljet_den = new TH2D("ljet_den","ljet_den",9,ptbins,3,etabins);
    TH2D *ljet_num = new TH2D("ljet_num","ljet_num",9,ptbins,3,etabins);
    ...
    // Independent check
    Long64_t nBcentral = 0, nBcentralTagged = 0, nBcentralUntagged = 0;
    double wBcentral = 0., wBcentralTagged = 0., wBcentralUntagged = 0.;
    ...
    for (Long64_t i=0; i<entries; ++i) {
        ...
        if (i%1000000==0)
            std::cout << "Processing " << i << "/" << entries << " (" << 100.*i/entries << "%)" << std::endl;
        ...
        for (int jet=0; jet<nCleanJet; ++jet) {
            ...
            if (flavour==5 && CleanJet_pt[jet]>30. && CleanJet_pt[jet]<100. && std::abs(CleanJet_eta[jet])<1.479) {
                ++nBcentral;
                wBcentral += XSWeight;

                if (tagged) {
                    ++nBcentralTagged;
                    wBcentralTagged += XSWeight;
                } else {
                    ++nBcentralUntagged;
                    wBcentralUntagged += XSWeight;
            }
            }
    ...
    std::cout << "\n=== CENTRAL b JETS 30-100 GeV ===\n"
              << "raw total     = " << nBcentral << "\n"
              << "raw tagged    = " << nBcentralTagged << "\n"
              << "raw untagged  = " << nBcentralUntagged << "\n"
              << "raw efficiency = " << double(nBcentralTagged)/nBcentral << "\n\n"
              << "weighted total     = " << wBcentral << "\n"
              << "weighted tagged    = " << wBcentralTagged << "\n"
              << "weighted untagged  = " << wBcentralUntagged << "\n"
              << "weighted efficiency = " << wBcentralTagged/wBcentral << std::endl;

    // Exact same region from histogram: pT bins 30-50, 50-70, 70-100 and central eta bin
    const double histCentralDen = bjet_den->Integral(2,4,2,2);
    const double histCentralNum = bjet_num->Integral(2,4,2,2);

    std::cout << "\n=== HISTOGRAM CROSS-CHECK ===\n"
              << "hist weighted total  = " << histCentralDen << "\n"
              << "hist weighted tagged = " << histCentralNum << "\n"
              << "hist efficiency      = " << histCentralNum/histCentralDen << "\n"
              << std::endl;

    // Efficiencies
    TFile *outfile = new TFile(fname,"RECREATE");

    TH2D *bjet_eff = (TH2D*)bjet_num->Clone("bjet_eff"); bjet_eff->Divide(bjet_den);
    TH2D *cjet_eff = (TH2D*)cjet_num->Clone("cjet_eff"); cjet_eff->Divide(cjet_den);
    TH2D *ljet_eff = (TH2D*)ljet_num->Clone("ljet_eff"); ljet_eff->Divide(ljet_den);
